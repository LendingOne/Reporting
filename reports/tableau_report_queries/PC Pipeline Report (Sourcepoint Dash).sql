...
dateframe_bank_closings as (SELECT date_value::date as dateframe
    FROM   date_series_days
...
bank_closings_dateframe2 as (SELECT dateframe
, LEAD(dateframe, 1) OVER (ORDER BY dateframe asc) as pos_1
, LEAD(dateframe, 2) OVER (ORDER BY dateframe asc) as pos_2
from dateframe_bank_closings
),

bank_closings_dateframe3 as (SELECT DISTINCT d.*, apps.id as application_id
from bank_closings_dateframe2 d
left join ods.ods_los.applications apps
where dateframe=current_date
),
...
initial_underwriting_date as (
    select
    a.id as application_number
    ,min(date(events.created_at)) as initial_underwriting_date
    from ods.ods_los.applications a 
    left join ods.ods_los.events on a.id=events.application_id
    where events.state  = 'initial_underwriting'
    group by a.id 
),
...
closing_kpi as (
    ...
    a.id as application_number
    ,case  
        when (a.closing_status is null or a.closing_status in ('likely','not_confirmed'))  and (date(l.settlement_date) = dt.pos_2 or date(l.settlement_date)= dt2.pos_2) then '2 Business Days'
        when (a.closing_status is null or a.closing_status in ('likely','not_confirmed'))  and (date(l.settlement_date) = dt.pos_1 or date(l.settlement_date)= dt2.pos_1) then 'Next Business Day' 
    ...
    from ods.ods_los.applications a 
        ...
        left join dateframe3 dt on a.id=dt.application_id
        left join bank_closings_dateframe3 dt2 on a.id = dt2.application_id
...
borrower_follow_up_kpi as (
    ...
    from ods.ods_los.applications a 
            ...
            left join initial_underwriting_date ts on ts.application_number = a.id 
            left join dateframe3 dt on a.id=dt.application_id     
            where a.state not in ('application','quote','incomplete_app','prelminary_review','lead','setup_servicing','closed','clear_to_close')
            and (a.closing_status is null or a.closing_status in ('likely','not_confirmed')) 
            and  ts.initial_underwriting_date is not null 
),


initial_underwriting_kpi as (

    select
    a.id as application_number
        ,case 
        when (a.state in ('initial_underwriting','initial_terms','initial_signed_term_sheet','final_terms','final_signed_term_sheet') and date(a2.term_sheet_reviewed_with_borrower_date) is null and ts.initial_underwriting_date <= dt.neg_3) 
             or (a.state in ('initial_underwriting','initial_terms','initial_signed_term_sheet','final_terms','final_signed_term_sheet') and date(a2.term_sheet_reviewed_with_borrower_date) is null and date(l.settlement_date) >= current_date and date(l.settlement_date) <= dt.pos_7 and date(ts.initial_underwriting_date) = dt.neg_2) then 'Past Due'
        when (a.state in ('initial_underwriting','initial_terms','initial_signed_term_sheet','final_terms','final_signed_term_sheet') and date(a2.term_sheet_reviewed_with_borrower_date) is null and ts.initial_underwriting_date <= dt.neg_2) 
             or (a.state in ('initial_underwriting','initial_terms','initial_signed_term_sheet','final_terms','final_signed_term_sheet') and date(a2.term_sheet_reviewed_with_borrower_date) is null and date(l.settlement_date) >= current_date and date(l.settlement_date) <= dt.pos_7 and date(ts.initial_underwriting_date) = dt.neg_1) then  'Due Today'
        when (a.state in ('initial_underwriting','initial_terms','initial_signed_term_sheet','final_terms','final_signed_term_sheet') and date(a2.term_sheet_reviewed_with_borrower_date) is null and ts.initial_underwriting_date <= dt.neg_1) 
             or (a.state in ('initial_underwriting','initial_terms','initial_signed_term_sheet','final_terms','final_signed_term_sheet') and date(a2.term_sheet_reviewed_with_borrower_date) is null and date(l.settlement_date) >= current_date and date(l.settlement_date) <= dt.pos_7 and ts.initial_underwriting_date = current_date) then 'Due Tomorrow'
        end as initial_underwriting_kpi   
    from ods.ods_los.applications a 
            left join ods.ods_los.loans l on a.id = l.application_id
            left join latest_follow_up_borrower bf on bf.loan_id = l.id
            left join initial_underwriting_date ts on ts.application_number = a.id 
...
kpi as (select 'PC PIPELINE' AS data_name
    , concat(c.first_name, ' ',c.last_name) as project_coordinator
    ,concat(ct.first_name, ' ',ct.last_name) as title_reviwer 
...
, concat(ct3.first_name,' ',ct3.last_name) closing_coordinator_name
from ods.ods_los.applications a 
    left join ods.ods_los.contacts c on a.project_coordinator_id = c.id 
    left join ods.ods_los.loans l on a.id = l.application_id
    left join latest_follow_up_title tf on tf.loan_id = l.id 
    ...
    left join initial_underwriting_kpi ik on ik.application_number = a.id 
    ...
    left join ods.ods_los.application_legal_closing_informations al on al.application_id = a.id 
    left join initial_underwriting_date td on td.application_number = a.id
    ...
    left join ods.ods_los.contacts ct on ct.id = a.title_reviewer_id
    left join ods.ods_los.contacts ct3 on a.paralegal_id=ct3.id
where a.rejected_at is null 
    and a.type in ('Application::Bridge','Application::Rental')
    and a.withdrawn_at is null
    and (t.team = 'PC')
    and l.properties_count = 1
...
max_fnd as (SELECT application_id,
max(created_at) as max_fnd_dt
from ods.ods_los.funding_shield_transactions
...
group by 1)
     ...
     select *
, null as preliminary_commitment_received_date
, null as funding_shield_status
, null as funding_shield_time_condition
, null as ops_product
     from kpi
     where kpi.row_num = 1
...
SELECT 'FUNDING SHIELD' AS data_name 
, concat(ct2.first_name,' ',ct1.last_name) project_coordinator_name
, concat(ct1.first_name,' ',ct1.last_name) title_reviewer_name
, loans.internal_id as loan_number
, null as appraisal_paid_at
, null as kickoff_call_with_borrower_date
, lgl.application_id as application_number
, null as created_at
, apps.state as closing_status
, apps.type as type
, null as in_conditions_review_from
, null as requested_for_ctc
, current_timestamp
, apps.id as application_id
, null as borrower_full_up_date
, null as borrower_follow_up_type
, lf.date as title_follow_up_date
, null as title_follow_up_type
, apps.state
, loans.settlement_date as closing_date
, null as closing_kpi
, null as follow_up_kpi
, null as transaction_purpose
    ,null as initial_underwriting_kpi
    ,null as Title_Order
    ,null as Title_Order_Confirmation
    ,null as title_follow_up
    ,null as appraisal_kpi
    ,null as processing_times_rental
    ,null as progressing_times_bridge
    ,null as processing_times_new_construction
    ,null as processing_times_portfolio
    ,null as cda_kpi 
    ,null as preliminary_review_welcome_call_kpi
    ,null as preliminary_review_follow_up_call_1_kpi
    ,null as preliminary_review_follow_up_call_2_kpi
    ,null as borrower_name
    ,null as row_num
, concat(ct3.first_name,' ',ct3.last_name) closing_coordinator_name
, lgl.preliminary_commitment_received_date
, CASE WHEN fnd.transaction_status=10 THEN 'In Process'
WHEN fnd.transaction_status=20 THEN 'Issues Reported'
WHEN fnd.transaction_status=30 THEN 'Approved'
WHEN fnd.transaction_status=40 THEN 'Approved / Archived'
WHEN fnd.transaction_status=50 THEN 'Rejected / Archived'
ELSE NULL END AS funding_shield_status
, CASE WHEN date(preliminary_commitment_received_date)=current_date THEN 'Due today'
WHEN date(preliminary_commitment_received_date)<=current_date THEN 'Past Due'
ELSE 'OTHER' END AS funding_shield_time_condition
, prod.ops_product
...
left join ods.ods_los.contacts ct2 on apps.project_coordinator_id=ct2.id
left join ods.ods_los.contacts ct3 on apps.paralegal_id=ct3.id
left join latest_follow_up_title lf on loans.id=lf.loan_id
left join ods.ods_sales_roster.deal_product_channel prod on lgl.application_id=prod.application_id
inner join max_fnd on fnd.application_id=max_fnd.application_id and fnd.created_at=max_fnd.max_fnd_dt
where (fnd.transaction_status=20 OR fnd.transaction_status is null)
AND apps.withdrawn_at is null AND apps.rejected_at is null AND lgl.preliminary_commitment_received_date is not null
and apps.state not in ('closed', 'setup_servicing','application','quote','incomplete_app','lead','preliminary_review')